...
# Valid camera numbers and configurations
valid_camera_numbers = [1, 2, 3, 4, 5, 6, 7, 8, 12, 13]
rightCamera = [5, 12, 13]

# Points to select on the image
points = {
    (195, 145): 0,
    (195, 355): 0,
    (340, 355): 0,
...
def unifyImages(img1, img2, rightCameraFlag):
    ...
    # Get dimensions of the resized court image
    ...
    img1 = add_legend(img1, rightCameraFlag)

    return img1

def add_legend(image, rightCameraFlag):
    # Text properties
    ...
    for i, item in enumerate(legend_items):
        y_position = first_y_offset + i * y_offset

        # Draw symbols
        if item["text"] == "Position of camera":
            triangle_side = 13
            triangle_points = np.array([
                [x_offset, y_position - triangle_side],
...
def edit_image(image, camera_number=1):
    ...
    for camera in camera_coordinates_visual:
        if camera == camera_number:
            ...
            # Draw triangle to represent camera
            triangle_side = 20
            ...
            cv2.fillPoly(image, [trianglePoints], color=(139, 139, 0))
            break
...
def takePoints(imageUndistorted, courtImg, camera_number, rightCameraFlag):
    ...
    for point in points:
        if points[point] == 0:
            while True:
                courtImgEdited = edit_image(courtImg.copy(), camera_number)
                image = unifyImages(img_copy.copy(), courtImgEdited, rightCameraFlag)

                cv2.imshow(f"{window_name} camera {camera_number}", image)
                key = cv2.waitKey(1) & 0xFF

                if clicked_point:
                    # Update point with clicked coordinates
                    points[point] = 1
    ...
    for video in videos:
        ...
        while True:
            ...
            if key == ord('s'):
                ...
                # Save points to the global dictionary
